...
namespace kv_store::disk {
    ...
    void FreeList::init (uint32_t size, const std::string & slabPath) {
        ...
        if (ftruncate (fileno (this-> _handle), size) != 0) {
            throw std::runtime_error ("Failed to resize freelist file : " + path);
        }
    ...
    void FreeList::increaseSize (uint32_t size) {
        ...
        if (oldSize < size) {
            if (ftruncate (fileno (this-> _handle), size) != 0) {
                throw std::runtime_error ("Failed to resize freelist file : "
    ...
    bool FreeList::isEmpty () const {
        ...
        if (head == __CONTENT_OFFSET__) {
            auto r = fread (&head, sizeof (uint32_t), 1, this-> _handle);
            return head == size;
        }
